...
let hrImage=document.querySelectorAll('hr.image').forEach(ele=>ele.style.border="2px solid purple");


// Hide the box number 17 (last box).
document.querySelector('div.archive').lastElementChild.remove();
...
let parent=document.querySelector('.archive');
let count=parent.children[0].childElementCount;
for(let i=0;i<count;i++)
{
  parent.children[0].children[0].remove();
}
// Replace all the elements inside box 1 with the para (you created above)
parent.children[0].append(para);
/* Walking the DOM
...
let box2Para=document.createElement('p');
...
let box1Para=document.querySelector('.box.one').firstElementChild.style.fontSize="0.8rem";
// Change the background of all the alternate boxes (1, 3, 5, ....) to aliceblue
let alernateBox=document.querySelectorAll('.box');
for(let i=0;i<alernateBox.length;i=i+2)
{
  alernateBox[i].style.backgroundColor="aliceblue";
}
...
let anchorOne=document.createElement('a').innerText="AltCampus";
let anchorTwo=document.createElement('a').innerText="Google";
